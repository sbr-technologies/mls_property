...
//            $contactPropertyObj        =   PropertyContact::find()->where(['property_id' => $propertyId])->one();
//            if(!empty($contactPropertyObj)){
//                $contactProperty    =   PropertyContact::find()->where(['property_id' => $propertyId])->all();
//            }
//            //\yii\helpers\VarDumper::dump($contactProperty); exit;
//            Yii::$app->response->format = Response::FORMAT_JSON;
...
//                $loopCnt++;
//                try {
//                    PropertyContact::deleteAll(['property_id' => $propertyId]);
//                    $saveCnt++;
//                } catch (Exception $ex) {
...
//            \yii\helpers\VarDumper::dump($propertyId."++".$contactFor); exit;
    ...
    public function actionIndex(){
        ...
        $searchModel->is_seller_information_show = 0;
    ...
    public function actionIndexJson(){
        ...
        foreach ($dataProvider->models as $model) {
                $finalArray[] = ['id' => $model->id, 'text' => $model->formattedAddress];
        }
        return ['result' => $finalArray];
    }
    
    
    public function actionBrokerageList(){
        $searchModel = new PropertySearch();
        $searchModel->is_seller_information_show = 1;
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render('brokerage-list', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
