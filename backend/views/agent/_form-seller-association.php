...
echo '<label class="control-label">Select Seller</label>';
...
echo Typeahead::widget([
    ...
    'dataset' => [
        [
            'remote' => [
                'url' => Url::to(['agent/seller-list']) . '&q=%QUERY',
                'wildcard' => '%QUERY'
            ],
    ...
    'pluginEvents' => [
        "typeahead:select" => "function(e, data) {
            $.loading();
            console.log(data);
            $.post('". Url::to(['agent-seller-mapping/create']). "', {agent_id:$model->id, seller_id:data.id}, function(response){
