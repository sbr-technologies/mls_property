    ...
    public function behaviors(){
        return [
            'access' => [
                ...
                'only' => ['my-subscriptions', 'plans', 'plan-details'],
                'rules' => [
                    [
//                        'actions' => ['logout'],
                        'allow' => true,
    ...
    public function actionMySubscriptions(){
        $user = Yii::$app->user->identity;
        $officeSubscription = [];
        $activeSubscription = Subscription::find()->where(['user_id' => $user->id])->active()->exists();
        $subscriptions = Subscription::find()->where(['user_id' => $user->id])->orderBy(['service_category_id' => SORT_ASC, 'id' => SORT_DESC])->all();
        if(\frontend\helpers\AuthHelper::is('agent') && $user->agency_id){
            $officeSubscription = Subscription::find()->where(['agency_id' => $user->agency_id])->active()->one();
        }
        return $this->render('my-subscriptions', ['subscriptions' => $subscriptions, 'activeSubscription' => $activeSubscription, 'officeSubscription' => $officeSubscription]);
    }
    ...
    public function actionPlanDetails($id, $duration = null){
        $model = PlanMaster::find()->where(['id' => $id, 'status' => 'active'])->one();
        if(empty($model)){
            throw new NotFoundHttpException('The requested page does not exist.');
        }
	$user = Yii::$app->user->identity;
        $oldLSubs = Subscription::find()->where(['user_id' => $user->id, 'service_category_id' => $model->service_category_id])->active()->exists();
        if($oldLSubs){
            throw new UnauthorizedHttpException('You already have an active subscription');
        }
        if(\frontend\helpers\AuthHelper::is('agent') && $user->agency_id){
            $agencySubs = Subscription::find()->where(['agency_id' => $user->agency_id, 'service_category_id' => $model->service_category_id])->active()->exists();
            if($agencySubs){
                throw new UnauthorizedHttpException('You already have an active Office Subscription. Pls contact your Office Manager');
            }
        }
...
//        print_r($paypalResult);die();
        if($paypalResult['ACK'] == 'Success'){
            ...
            return $this->redirect(['subscription/payment-success', 'transactionid' => $paypalResult['TRANSACTIONID'], 'subscriptionid' => $subscription->id]);
        }
        Yii::$app->session->setFlash('error', 'Transaction could not completed at this time try again later');
        return $this->redirect(['subscription/plan-details', 'id' => $plan->id]);
    }
	...
	$user = Yii::$app->user->identity;
        ...
        $SECFields = array(
            ...
            'returnurl' => Url::to(['/subscription/do-express-checkout'], true), // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
            'cancelurl' => Url::to(['/subscription/cancel-payment'], true), // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
...
//        }
        ...
        $subsStart = strtotime('now');
//        $oldSubs = Subscription::find()->where(['user_id' => Yii::$app->user->id, 'service_category_id' => $plan->service_category_id, 'plan_id' => $plan->id])->active()->orderBy(['id' => SORT_DESC])->one();
//        if(!empty($oldSubs)){
//            $oldSubs->status = 'renewed';
//            $subsStart = $oldSubs->subs_end;
//            $oldSubs->save();
//        }else{
//            $oldUpSubs = Subscription::find()->where(['user_id' => Yii::$app->user->id, 'service_category_id' => $plan->service_category_id])->andWhere(['<', 'paid_amount', $paypalResult['AMT']])->active()->orderBy(['id' => SORT_DESC])->one();
//            if(!empty($oldUpSubs)){
//                $oldUpSubs->status = 'upgraded';
//                $oldUpSubs->save();
//            }
//        }
...
//            ->send();
    ...
    public function actionTerminate($id){
        $user = Yii::$app->user->identity;
        $subscription = Subscription::find()->where(['id' => $id, 'user_id' => $user->id])->active()->one();
        if(empty($subscription)){
        ...
        $subscription->save();
        Yii::$app->session->setFlash('success', 'Your current subscription has been terminated');
        return $this->redirect(['my-subscriptions']);
    }
