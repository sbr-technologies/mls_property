...
$this->title = "Forclouser";

$blockLocation = 1;
$profileId = 5;
$propertyType   = PropertyType::findByTitle('Foreclosures');
$staticPage     = StaticPage::findByName('Home');
$foreclosureProperties = Property::find()->where(['property_type_id' => $propertyType->id])->active()->all();

$staticBlockTpe     = StaticBlockLocationMaster::findByTitle('Foreclosures');
...
<section>
    ...
    <div class="home-listing-sec property-buy-listing-sec">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <h2>Foreclosures <span>See all in London, KY</span></h2>
                    <div class="listing-item-sec">
                        <?php
                        if (!empty($foreclosureProperties)) {
                        ?>
                        <div class="row">
                            <div id="newest-listings-buy" class="owl-carousel owl-theme">
                                <?php
                                foreach ($foreclosureProperties as $key => $property) {
                                ?>
                                <div class="item">
