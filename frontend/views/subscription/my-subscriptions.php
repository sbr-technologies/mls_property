...
<div class="content-wrapper">
  ...
  <section class="content">
            ...
            <table class="table table-bordered text-center">
              <tbody>
                ...
                <td class="text-capitalize"><?php if($subs->status == 'active'){?>
                    <a href="<?= \yii\helpers\Url::to(['subscription/terminate', 'id' => $subs->id])?>" onclick="return confirm('Are you sure want to terminate your current subscription?');">Terminate</a>
                <?php }?>
                </td>
                <?php }}else{ ?>
    ...
    <?php if(!empty($officeSubscription)){?>
      <div class="col-sm-12">
        <div class="box box-default box-solid table-listing">
          <div class="box-header with-border">
            <h3 class="box-title">Office Subscription</h3>
          </div>
          <div class="box-body table-responsive">
            <table class="table table-bordered text-center">
              <tbody>
                ...
                <tr>
                <td>1</td>
                <td><?=$officeSubscription->plan->serviceCategory->name?></td>
                <td><?=$officeSubscription->plan->title?></td>
                <td><?= Yii::$app->formatter->asDate($officeSubscription->subs_start)?></td>
                <td><?= Yii::$app->formatter->asDate($officeSubscription->subs_end)?></td>
                <td class="text-capitalize"><?=$officeSubscription->status?></td>
                </tr>
